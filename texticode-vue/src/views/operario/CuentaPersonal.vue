  ...
  <div style="display:flex;min-height:100vh;background:#f1f5f9;position:relative;overflow:hidden">
    ...
    <main class="main">
      <div class="hero-header" :class="{ 'hero-in': mounted }">
        ...
        <div class="hero-text">
          <h1 class="hero-title">
            <span v-for="(c, i) in titleChars" :key="i" class="title-char" :class="{ 'char-visible': mounted }" :style="{ transitionDelay: mounted ? i * 35 + 'ms' : '0ms' }">{{ c === ' ' ? '\u00A0' : c }}</span>
          </h1>
      ...
      <div v-else-if="error" class="error-banner">
        ...
        {{ error }}<button class="btn-retry" @click="cargarDatos">Reintentar</button>
      </div>
 
      <template v-else>
        <div v-if="!auth.idUsuario" class="empty-session">
          <div class="empty-icon-hex">
            <svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#9ca3af"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"/></svg>
          </div>
          <p class="empty-title">Ingresa con tu cuenta</p>
          <p class="empty-sub">Inicia sesión para ver tu perfil y comprobantes.</p>
        </div>
 
          <!-- BANNER PERFIL idéntico al operario -->
        <template v-else>
          ...
          <!-- STATS -->
          <div class="stats-row">
            <div class="stat-card" v-for="(s, i) in statsCards" :key="s.label" :style="{ '--i': i }">
              <div class="stat-accent" :style="{ background: s.color }"></div>
              <div class="stat-icon-bg" :style="{ color: s.color }">
                <svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke-width="1.2" stroke="currentColor" shape-rendering="geometricPrecision" style="display:block"><path stroke-linecap="round" stroke-linejoin="round" :d="s.icon"/></svg>
              </div>
              <div class="stat-body">
          ...
          <!-- COMPROBANTES -->
          <div class="card" style="--d:420ms">
            <div class="card-header">
              <div class="ch-dot" style="background:#e0eaf2">
                <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#1f3a52" shape-rendering="geometricPrecision" style="display:block"><path stroke-linecap="round" stroke-linejoin="round" d="m9 14.25 6-6m4.5-3.493V21.75l-3.75-1.5-3.75 1.5-3.75-1.5-3.75 1.5V4.757c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0c1.1.128 1.907 1.077 1.907 2.185Z"/></svg>
              </div>
              Mis Comprobantes de Entrega
              <div class="card-badge">{{ ordenesOrdenadas.length }}</div>
              <div class="card-line"></div>
            </div>
            <div v-if="cargandoComprobantes" class="loading-inline">
            ...
            <template v-else>
              <table>
                <thead>
                  <tr>
                    ...
                    <th>Producto</th><th>Estado</th><th>Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  <TransitionGroup name="row">
                    <tr v-for="(o, idx) in ordenesOrdenadas" :key="o.id" class="table-row" :class="{ 'row-flash': o.flash }" :style="{ animationDelay: idx * 45 + 'ms' }">
                      ...
                      <td>
                        <div class="actions">
                          <button class="action-btn view-btn" title="Ver detalle" @click="verDetalle(o)">
                            <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" shape-rendering="geometricPrecision" style="display:block"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/></svg>
                          </button>
                          <button class="action-btn download-btn" title="Descargar" @click="descargar(o)">
                            <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" shape-rendering="geometricPrecision" style="display:block"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
                          </button>
                        </div>
                      </td>
                    </tr>
              ...
              <div v-if="ordenesOrdenadas.length === 0" class="empty-msg">No tienes comprobantes registrados aún.</div>
            </template>
          </div>
        </template>
      </template>
    ...
    <Transition name="modal">
      <div v-if="ordenSeleccionada" class="modal-overlay" @click.self="ordenSeleccionada = null">
        <div class="modal-container">
          <div class="modal-header">
            <h3 class="modal-title">Orden #{{ ordenSeleccionada.numero }}</h3>
            <button class="modal-close" @click="ordenSeleccionada = null"><svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
          </div>
          <div class="modal-body detalle-grid">
            ...
            <div class="detalle-item"><span class="detalle-label">Estado</span><span class="badge" :class="ordenSeleccionada.estadoClass">{{ ordenSeleccionada.estado }}</span></div>
          </div>
      ...
      <div v-if="modalVisible" class="modal-overlay" @click.self="cerrarModal">
        <div class="modal-container">
          <div class="modal-header">
            ...
            <button class="modal-close" @click="cerrarModal"><svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
          </div>
...
const mounted = ref(false), cargando = ref(true), cargandoComprobantes = ref(false)
const error = ref(''), errorGuardar = ref(''), guardando = ref(false)
const modalVisible = ref(false), mostrarPassword = ref(false)
const toast = ref({ visible: false, msg: '', type: 'success' })
const ordenSeleccionada = ref(null), perfil = ref({}), ordenes = ref([]), formEdicion = ref({})
const displayOrdenes = ref(0), displayEntregadas = ref(0), displayPendientes = ref(0)
...
function animateCount(targetRef, target) {
  ...
  const id = setInterval(() => { val += step; if (val >= target) { targetRef.value = target; clearInterval(id) } else targetRef.value = val }, 20)
}
 
const iniciales = computed(() => (perfil.value.Nombre_Completo || '').split(' ').map(p => p[0]).slice(0, 2).join('').toUpperCase())
...
const statsCards = computed(() => [
  { label: 'Órdenes totales', display: displayOrdenes.value,    color: '#1f3a52', icon: ICON_ORDERS },
  { label: 'Entregadas',      display: displayEntregadas.value, color: '#16a34a', icon: ICON_CHECK  },
  { label: 'Pendientes',      display: displayPendientes.value, color: '#d97706', icon: ICON_CLOCK  },
])
...
const sortKeyOrdenes = ref('numero'), sortDirOrdenes = ref(1)
function sortOrdenes(key) { sortKeyOrdenes.value === key ? (sortDirOrdenes.value *= -1) : (sortKeyOrdenes.value = key, sortDirOrdenes.value = 1) }
function sortIcon(key) { if (sortKeyOrdenes.value !== key) return '⇅'; return sortDirOrdenes.value === 1 ? '↑' : '↓' }
const ordenesOrdenadas = computed(() => [...ordenes.value].sort((a, b) => { const k = sortKeyOrdenes.value; return (a[k] > b[k] ? 1 : a[k] < b[k] ? -1 : 0) * sortDirOrdenes.value }))
...
async function cargarDatos() {
  ...
  try {
    ...
    try {
      const [dataOrdenes, dataComprobantes] = await Promise.all([getOrdenesDeCliente(auth.idUsuario), getComprobantes()])
      const mapaComp = {}; dataComprobantes.forEach(c => { mapaComp[c.Id_Orden] = c })
      ordenes.value = (Array.isArray(dataOrdenes) ? dataOrdenes : []).map(o => {
        const comp = mapaComp[o.Id_Orden]; const estadoComp = comp?.Estado || o.Estado
        const esEntregado = estadoComp === 'Entregado' || o.Estado === 'Completada'
        return { id: o.Id_Orden, numero: String(o.Id_Orden).padStart(4, '0'), cliente: perfil.value.Nombre_Completo || '', fecha: o.Fecha_Limite?.split('T')[0] || '—', productos: o.Producto || o.Descripcion || '—', estado: esEntregado ? 'Entregado' : 'Pendiente', estadoClass: esEntregado ? 'success' : 'danger', flash: false }
      })
    } catch { ordenes.value = [] } finally { cargandoComprobantes.value = false }
  } catch (e) { error.value = e.message || 'Error al cargar el perfil.' }
  finally {
    cargando.value = false
    setTimeout(() => { mounted.value = true; animateCount(displayOrdenes, ordenes.value.length); animateCount(displayEntregadas, ordenes.value.filter(o => o.estadoClass === 'success').length); animateCount(displayPendientes, ordenes.value.filter(o => o.estadoClass === 'danger').length) }, 80)
  }
}
onMounted(cargarDatos)
function verDetalle(o) { ordenSeleccionada.value = o }
function descargar(o) { o.flash = true; setTimeout(() => { o.flash = false }, 1200); mostrarToast('Descargando comprobante ' + o.numero + '...') }
...
async function guardarCambios() {
  ...
  } catch (e) { errorGuardar.value = e.message || 'Error al guardar.' } finally { guardando.value = false }
}
function mostrarToast(msg, type = 'success') { toast.value = { visible: true, msg, type }; setTimeout(() => { toast.value.visible = false }, 2800) }
</script>
 
<style scoped>
.main { flex:1;padding:28px 30px;overflow-y:auto;position:relative;background:radial-gradient(circle at 20% 10%,rgba(37,99,235,.08),transparent 40%),radial-gradient(circle at 80% 70%,rgba(31,58,82,.08),transparent 45%); }
...
.bg-grid { position:fixed;inset:0;pointer-events:none;z-index:0;opacity:.5;background-image:linear-gradient(rgba(31,58,82,0.06) 1px,transparent 1px),linear-gradient(90deg,rgba(31,58,82,0.06) 1px,transparent 1px);background-size:40px 40px; }
.hero-header { position:relative;z-index:1;display:flex;align-items:center;gap:20px;margin-bottom:24px;opacity:0;transform:translateY(-14px);transition:opacity .5s ease,transform .5s cubic-bezier(.34,1.56,.64,1); }
...
.hero-icon-box { width:52px;height:52px;border-radius:14px;background:linear-gradient(135deg,#1f3a52,#2d5f8a);display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(31,58,82,.35);position:relative;z-index:2; }
...
.profile-banner { position:relative;z-index:1;overflow:hidden;background:linear-gradient(135deg,#111827 0%,#1f3a52 55%,#0f2236 100%);border-radius:20px;padding:28px 30px;display:flex;align-items:center;gap:22px;margin-bottom:20px;opacity:0;transform:translateY(22px) scale(.97);transition:opacity .6s ease,transform .6s cubic-bezier(.34,1.56,.64,1); }
...
.avatar-wrap { position:relative;width:84px;height:84px;flex-shrink:0;display:flex;align-items:center;justify-content:center;opacity:0;transform:scale(.4) rotate(-20deg);transition:opacity .6s ease .1s,transform .7s cubic-bezier(.34,1.56,.64,1) .1s; }
.avatar-wrap.avatar-in { opacity:1;transform:none; }
.avatar-hex-border { position:absolute;inset:-10px;width:calc(100% + 20px);height:calc(100% + 20px); }
.avatar-circle { width:80px;height:80px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:800;position:relative;z-index:2;border:2.5px solid rgba(255,255,255,0.25);box-shadow:0 0 0 3px rgba(255,255,255,.1),0 8px 24px rgba(0,0,0,.35);letter-spacing:1px; }
.profile-info { flex:1; }
.profile-name { font-size:22px;font-weight:800;color:white;display:flex;flex-wrap:wrap; }
...
.btn-edit { display:inline-flex;align-items:center;gap:6px;background:rgba(255,255,255,.1);color:white;border:1px solid rgba(255,255,255,.2);border-radius:10px;padding:9px 16px;font-size:13px;font-weight:600;cursor:pointer;backdrop-filter:blur(8px);transition:background .2s,transform .2s cubic-bezier(.34,1.56,.64,1);animation:slideX .5s ease .3s both; }
.btn-edit:hover { background:rgba(255,255,255,.2);transform:translateY(-2px); }
@keyframes slideX { from{opacity:0;transform:translateX(-10px)} to{opacity:1;transform:none} }
.stats-row { position:relative;z-index:1;display:flex;gap:16px;margin-bottom:20px; }
.stat-card { flex:1;position:relative;overflow:hidden;display:flex;align-items:center;gap:16px;background:white;border:1px solid #e5e7eb;border-radius:14px;padding:18px 20px;opacity:0;transform:translateY(18px) scale(.96);animation:statIn .5s cubic-bezier(.34,1.56,.64,1) calc(var(--i, 0) * 90ms + 160ms) both;transition:box-shadow .2s,transform .2s; }
...
.card { position:relative;z-index:1;background:white;border:1px solid #e5e7eb;border-radius:14px;padding:22px 24px;margin-bottom:16px;opacity:0;transform:translateY(16px);animation:slideUp .5s ease var(--d,300ms) both;transition:box-shadow .2s; }
...
.contact-item { display:flex;align-items:center;gap:10px;padding:11px 13px;border-radius:10px;border:1px solid #f3f4f6;background:#fafbfc;opacity:0;transform:translateY(8px);animation:contactReveal .4s ease both;transition:background .2s,border-color .2s,transform .2s; }
...
.contact-icon-wrap { width:30px;height:30px;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:transform .2s cubic-bezier(.34,1.56,.64,1); }
.contact-item:hover .contact-icon-wrap { transform:scale(1.15) rotate(-5deg); }
.c-label { font-size:10px;color:#9ca3af;text-transform:uppercase;letter-spacing:.4px;margin-bottom:2px; }
...
.table-row { transition:background .18s;animation:rowSlideIn .35s ease both; }
...
tr:hover .order-num-pill { background:#1f3a52;color:white; }
.badge { padding:4px 10px;border-radius:999px;font-size:11px;font-weight:600; }
.success { background:#dcfce7;color:#15803d; }
.danger  { background:#fee2e2;color:#b91c1c; }
.actions { display:flex;gap:8px;align-items:center; }
.action-btn { width:32px;height:32px;border-radius:7px;border:none;background:#1f3a52;display:flex;align-items:center;justify-content:center;cursor:pointer;color:white;transition:all .15s; }
...
.skeleton-row { height:48px;border-radius:8px;background:linear-gradient(90deg,#f3f4f6 0%,#fafafa 40%,#f3f4f6 80%);background-size:300% 100%;animation:skeletonWave 1.6s ease-in-out infinite; }
...
.modal-container { background:white;border-radius:16px;width:460px;max-width:95vw;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.15);animation:modalIn .35s cubic-bezier(.34,1.56,.64,1) both; }
...
.detalle-label { font-size:11px;color:#9ca3af;text-transform:uppercase;letter-spacing:.5px; }
...
@media (max-width:960px) { .main{padding:20px 16px} .stats-row{flex-direction:column} .contact-grid{grid-template-columns:1fr} .profile-banner{flex-wrap:wrap} .btn-edit{width:100%;justify-content:center} }
.bg-orbs { position: fixed; inset: 0; pointer-events: none; z-index: 0; overflow: hidden; }
