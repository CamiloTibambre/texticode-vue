...
async function ensureGoogleSchema() {
  await db.execute(`CREATE TABLE IF NOT EXISTS google_calendar_tokens (
    ...
    CONSTRAINT fk_google_calendar_usuario FOREIGN KEY (Id_Usuario) REFERENCES usuario (Id_Usuario) ON DELETE CASCADE
  )`)

  const optionalColumns = [
    ['Google_Id', 'varchar(255) DEFAULT NULL AFTER Google_Email'],
    ['Last_Login_Google', 'datetime DEFAULT NULL AFTER Sync_Enabled'],
    ['Last_Sync_At', 'datetime DEFAULT NULL AFTER Last_Login_Google'],
  ]
  ...
  for (const [column, definition] of optionalColumns) {
    const [rows] = await db.execute(
      `SELECT COUNT(*) AS total
       ...
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'google_calendar_tokens' AND COLUMN_NAME = ?`,
      [column]
    )
    if (!rows[0].total) {
      await db.execute(`ALTER TABLE google_calendar_tokens ADD COLUMN ${column} ${definition}`)
    }
  }
}
...
async function refreshAccessToken(row) {
  ...
  const expiryDate = data.expires_in ? Date.now() + Number(data.expires_in) * 1000 : row.Expiry_Date
  await db.execute(
    `UPDATE google_calendar_tokens
     SET Access_Token = ?, Expiry_Date = ?, Scope = COALESCE(?, Scope), Token_Type = COALESCE(?, Token_Type)
     WHERE Id_Usuario = ?`,
...
async function upsertGoogleTokens(userId, profile, tokenPayload) {
  ...
  await db.execute(
    `INSERT INTO google_calendar_tokens
      (Id_Usuario, Google_Id, Google_Email, Access_Token, Refresh_Token, Scope, Token_Type, Expiry_Date, Calendar_Id, Sync_Enabled, Last_Login_Google)
     ...
     ON DUPLICATE KEY UPDATE
      ...
      Sync_Enabled = 1`,
    [userId, profile.id, profile.email, tokens.accessToken, tokens.refreshToken, tokens.scope, tokens.tokenType, tokens.expiryDate]
  )
}
...
router.get('/callback', async (req, res) => {
  ...
  try {
    if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) throw new Error('Google OAuth no está configurado en el servidor.')
    ...
    if (payload.action === 'link') {
      ...
      if (!user || user.Estado === 'inactivo') throw new Error('Tu usuario de Texticode no está activo.')
    ...
    const [rows] = await db.execute(
      `SELECT u.Id_Usuario, u.Nombre_Completo, u.Nombre_Usuario,
       ...
       INNER JOIN rol r ON r.Id_Rol = u.Id_Rol
       WHERE (g.Google_Id = ? OR g.Google_Email = ?)
       LIMIT 1`,
      [profile.id, profile.email]
    )

    if (!rows.length) throw new Error('Esta cuenta de Google aún no está vinculada a un usuario creado por el administrador.')
    const user = rows[0]
    if (user.Estado === 'inactivo') throw new Error('Tu cuenta está inactiva. Contacta al administrador.')
...
router.get('/status', requireAuth, async (req, res) => {
  await ensureGoogleSchema()
  const [rows] = await db.execute(
    `SELECT Google_Email, Scope, Calendar_Id, Sync_Enabled, Last_Login_Google, Last_Sync_At, Updated_At
...
router.delete('/unlink', requireAuth, async (req, res) => {
  await db.execute('DELETE FROM google_calendar_tokens WHERE Id_Usuario = ?', [req.auth.id])
  res.json({ mensaje: 'Cuenta de Google desvinculada correctamente.' })
})

router.patch('/settings', requireAuth, async (req, res) => {
  await ensureGoogleSchema()
  const syncEnabled = req.body.syncEnabled ? 1 : 0
  const calendarId = req.body.calendarId || 'primary'
  await db.execute(
    `UPDATE google_calendar_tokens SET Sync_Enabled = ?, Calendar_Id = ? WHERE Id_Usuario = ?`,
...
async function getConnectedGoogleRow(userId) {
  await ensureGoogleSchema()
  const [rows] = await db.execute('SELECT * FROM google_calendar_tokens WHERE Id_Usuario = ? LIMIT 1', [userId])
...
async function getUserOrders(auth) {
  ...
  if (Number(user.Id_Rol) === 1) {
    const [rows] = await db.execute(
      `SELECT op.Id_Orden, op.Producto, op.Descripcion, op.Cantidad, op.Prioridad, op.Estado,
              op.Fecha_Limite, op.Fecha_Creacion, c.Nombre_Completo AS Cliente, o.Nombre_Completo AS Operario
       FROM orden_produccion op
       LEFT JOIN usuario c ON c.Id_Usuario = op.Id_Cliente
       LEFT JOIN usuario o ON o.Id_Usuario = op.Id_Operario
       WHERE op.Fecha_Limite >= CURDATE()
  ...
  if (Number(user.Id_Rol) === 2) {
    const [rows] = await db.execute(
      `SELECT op.Id_Orden, op.Producto, op.Descripcion, op.Cantidad, op.Prioridad, op.Estado,
       ...
       LEFT JOIN usuario c ON c.Id_Usuario = op.Id_Cliente
       LEFT JOIN usuario o ON o.Id_Usuario = op.Id_Operario
       WHERE (op.Id_Operario = ? OR EXISTS (
         SELECT 1 FROM usuario_orden uo WHERE uo.Id_Orden = op.Id_Orden AND uo.Id_Usuario = ?
       )) AND op.Fecha_Limite >= CURDATE()
       ORDER BY op.Fecha_Limite ASC
       LIMIT 50`,
      [auth.id, auth.id]
    )
  ...
  const [rows] = await db.execute(
    `SELECT op.Id_Orden, op.Producto, op.Descripcion, op.Cantidad, op.Prioridad, op.Estado,
     ...
     LEFT JOIN usuario c ON c.Id_Usuario = op.Id_Cliente
     LEFT JOIN usuario o ON o.Id_Usuario = op.Id_Operario
     WHERE op.Id_Cliente = ? AND op.Fecha_Limite >= CURDATE()
     ORDER BY op.Fecha_Limite ASC
     LIMIT 50`,
    [auth.id]
  )
...
async function createOrUpdateCalendarEvent(accessToken, calendarId, order) {
  ...
  const listRes = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events?${params}`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  })
...
router.post('/sync/delivery-events', requireAuth, async (req, res) => {
  try {
    ...
    await db.execute('UPDATE google_calendar_tokens SET Last_Sync_At = NOW() WHERE Id_Usuario = ?', [req.auth.id])
...
router.get('/events/upcoming', requireAuth, async (req, res) => {
  try {
    ...
    const response = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(googleRow.Calendar_Id || 'primary')}/events?${params}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
